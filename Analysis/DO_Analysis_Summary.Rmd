...
one of the most extensive and best documented dataset from the Northeastern US 
...
review of LCWMD data. These analyses are based on their summary datasets, and
...
In this dataset a "TRUE" value consistently implies that water quality criteria
...
*  Recovery in DO several days later (1/28/2016) corresponds to a drop in
   chlorides, and a **brief** increase on water depth, but there is no related
...
Note that this conflicts with results of the analysis of exceedances, where both
...
that with the large datasets we have, lots of small signals are statistically
significant.
...
Let's try shifting the water temp to a linear term, and dropping the air temp
...
trends for some of those sites based on a limited number of years.
